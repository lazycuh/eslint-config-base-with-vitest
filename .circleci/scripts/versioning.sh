...
function bumpPackageVersion {
    if [ $(matchCommitMessagePattern "build|chore|ci|docs|fix|perf|refactor|style|test") != "0" ]; then
        ...
        echo "Commit message should start with a valid type, please see https://github.com/babybeet/eslint-config-base-with-vitest/blob/main/CONTRIBUTING.md to learn more"
        npm version minor --no-git-tag-version
    fi
